...
static const char *const FW_RELEASE_PROOF = "TamaPoke-2.34.0-official";
...
void serviceWifi() {
  ...
  if (status == WL_CONNECTED) {
    ...
    if (checkAfterConnect) onlineUpdate();
    return;
  }
...
void onlineUpdate() {
  ...
  // Cloudflare/WiFi 初次恢复时偶尔会丢掉第一条 TLS 请求，短暂重连后再试一次。
...
static void drawRoomDetails(uint8_t biome, uint32_t now, bool night) {
  ...
  // 固定四列五行的场景锚点，避免不同道具购买顺序改变位置或互相覆盖。
  static const int PROP_X[20] = {
    72, 180, 288, 396, 72, 180, 288, 396, 72, 180,
    288, 396, 72, 180, 288, 396, 72, 180, 288, 396
  };
  static const int PROP_Y[20] = {
    154, 154, 154, 154, 198, 198, 198, 198, 242, 242,
    242, 242, 286, 286, 286, 286, 330, 330, 330, 330
  };
  uint32_t props = pet.roomProps();
  for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; ++slot)
    if (props & (uint32_t)(1UL << slot)) drawPropProduct(slot, PROP_X[slot], PROP_Y[slot], 0.34f);
  (void)biome;
  (void)now;
}
...
void decorTap(int16_t x, int16_t y) {
  ...
  uint8_t pages = (uint8_t)((total + 7) / 8);
  if (!pages) pages = 1;
  if (y >= 392 && y <= 444 && pages > 1) {
    if (x < 150 && decorPage > 0) decorPage--;
    else if (x > 316 && decorPage + 1 < pages) decorPage++;
    return;
  }
  if (x < 24 || x > 430 || y < 54 || y >= 54 + 4 * 72) return;
  uint8_t col = x >= 233 ? 1 : 0;
  uint8_t row = (uint8_t)((y - 54) / 72);
  if (row > 3) return;
  int cardX = 24 + col * 210;
  if (x > cardX + 196 || y >= 54 + row * 72 + 54) return;
...
void renderDecor() {
  ...
  for (uint8_t i = 0; i < 8; ++i) {
    ...
    int x = 24 + (i % 2) * 210;
    int y = 54 + (i / 2) * 72;
    gfx->fillRoundRect(x, y, 196, 54, 9, C565(0x2f, 0x42, 0x58));
    gfx->drawRoundRect(x, y, 196, 54, 9, UI_WHITE);
    gfx->setTextColor(UI_WHITE);
    gfx->setTextSize(1);
    if (entry < 8) {
      uint8_t slot = entry;
      ...
      drawDecorIcon(slot, x + 21, y + 27, gNight);
      String name = wifiLabelFit(String(labels[slot]), 72);
      gfx->setCursor(x + 42, y + 8); gfx->print(name.c_str());
      gfx->setTextColor(owned ? (placed ? UI_BAR_OK : UI_WHITE) : C565(0xa8, 0xb0, 0xc0));
      gfx->setCursor(x + 42, y + 31); gfx->print(owned ? (placed ? "已布置" : "点击布置") : "未拥有");
      if (!owned) drawLockBadge(x + 177, y + 16);
    } else {
      uint8_t slot = propSlots[entry - 8];
      bool placed = (pet.roomProps() & (uint32_t)(1UL << slot)) != 0;
      drawPropProduct(slot, x + 20, y + 27, 0.34f);
      String name = wifiLabelFit(String(SHOP_PRODUCTS[SHOP_CAT_PROP][slot].name), 56);
      gfx->setTextColor(UI_WHITE);
      gfx->setCursor(x + 40, y + 8); gfx->print(name.c_str());
      const int bx[2] = { x + 104, x + 151 };
      const char *bl[2] = { "布置", "收回" };
      const uint16_t bc[2] = { UI_BAR_OK, UI_BAR_BAD };
      for (uint8_t b = 0; b < 2; ++b) {
        gfx->fillRoundRect(bx[b], y + 24, 43, 26, 6, placed && b == 0 ? C565(0xc8, 0xd9, 0xcd) : UI_WHITE);
        gfx->drawRoundRect(bx[b], y + 24, 43, 26, 6, UI_INK);
        gfx->setTextColor(bc[b]);
        gfx->setCursor(bx[b] + (43 - uiTextWidth(bl[b], 6)) / 2, y + 29); gfx->print(bl[b]);
      }
    }
  }
  ...
  gfx->setTextSize(1);
  if (pages > 1) {
    gfx->drawLine(74, 412, 98, 412, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(74, 412, 86, 402, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(74, 412, 86, 422, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(368, 412, 392, 412, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(392, 412, 380, 402, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(392, 412, 380, 422, gNight ? UI_WHITE : UI_INK);
  }
  char pageLabel[12]; snprintf(pageLabel, sizeof(pageLabel), "%u/%u", (unsigned)(decorPage + 1), (unsigned)pages);
  gfx->setCursor(CX - uiTextWidth(pageLabel, 6) / 2, 406); gfx->print(pageLabel);
  gfx->setCursor(CX - uiTextWidth(T(S_TAP_DECOR), 6) / 2, 438);
...
void warehouseTap(int16_t x, int16_t y) {
  ...
  if (shopPage == SHOP_CAT_EQUIPMENT && y >= 174 && y < 414) {
    ...
    for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; ++slot) {
      ...
      } else if (x >= 338 && x < 400) ok = pet.sellShopProduct(shopPage, slot);
      if (x >= 206 && x < 400) economyNotice(ok);
      return;
    }
    return;
  }
...
void renderWarehouse() {
  ...
  for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; slot++)
    if (equipmentPage ? warehouseEquipmentVisible(slot) : pet.warehouseCount(shopPage, slot)) ownedCount++;
  ...
  if (equipmentPage) {
    for (uint8_t row = 0; row < 5; ++row) {
      ...
      String itemName = wifiLabelFit(String(SHOP_PRODUCTS[shopPage][slot].name), 82);
      gfx->setTextColor(UI_INK); gfx->setTextSize(1);
      ...
      for (uint8_t i = 0; i < 3; ++i) {
        ...
        gfx->drawRoundRect(bx[i], y + 4, 62, 36, 6, UI_INK);
        gfx->setTextColor(colors[i]);
        gfx->setCursor(bx[i] + (62 - uiTextWidth(labels[i], 6)) / 2, y + 9);
        gfx->print(labels[i]);
      }
    }
