...
// Release 2.28.0: encrypted origin, device-bound tokens, and browser grants.
...
async function firmware(request, env, grantKind) {
  ...
  try {
    const data = await decryptFirmware(env, grantKind === "install" ? "tamapoke-2.28.0-merged.bin.enc" : "tamapoke-2.28.0-app.bin.enc");
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") return json({ ok: true, service: "tamapoke-license" });
