...
// Release 2.29.0: encrypted origin, device-bound tokens, and browser grants.
...
async function firmware(request, env, grantKind) {
  ...
  try {
    const data = await decryptFirmware(env, grantKind === "install" ? "tamapoke-2.29.0-merged.bin.enc" : "tamapoke-2.29.0-app.bin.enc");
...
async function deviceVersion(request, env) {
  ...
  return json({ version: "2.29.0", deviceId, update: (request.headers.get("x-tamapoke-version") || "") !== "2.29.0" });
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") {
      return json({ ok: true, service: "tamapoke-license", version: "2.29.0" });
    }
