...
bool onlineUpdatePending = false;
...
static const char *const FW_RELEASE_PROOF = "TamaPoke-2.36.0-official";
...
void serviceWifi() {
  if (!wifiConnecting) {
    if (onlineUpdatePending && WiFi.status() == WL_CONNECTED &&
  ...
  if (status == WL_CONNECTED) {
    ...
    bool checkAfterConnect = onlineUpdatePending;
...
void renderClock() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - uiTextWidth(ver, 12) / 2, 396);
  gfx->print(ver);
