...
// Release 2.36.0: visible-only paged WiFi picker and aligned settings controls.
...
async function firmware(request, env, grantKind) {
  ...
  try {
    const data = await decryptFirmware(env, grantKind === "install" ? "tamapoke-2.36.0-merged.bin.enc" : "tamapoke-2.36.0-app.bin.enc");
