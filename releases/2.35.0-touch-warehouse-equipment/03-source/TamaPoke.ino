...
static const char *const FW_RELEASE_PROOF = "TamaPoke-2.35.0-official";
...
void renderClock() {
  ...
  for (uint8_t i = 0; i < 3; i++) {
    ...
    gfx->setCursor(vx + (60 - uiTextWidth(vnames[i], 6)) / 2, 183); gfx->print(vnames[i]);
  }
  ...
  for (uint8_t i = 0; i < 3; i++) {
    ...
    gfx->setCursor(tx + (60 - uiTextWidth(touchNames[i], 6)) / 2, 233); gfx->print(touchNames[i]);
  }
