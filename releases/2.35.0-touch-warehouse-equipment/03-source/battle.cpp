...
uint8_t wildLevelFor(uint8_t petLevel, uint8_t luckRoll) {
  ...
  // 随晋升阶段逐步提高野外对手的平均等级，避免高等级宠物一直遇到
  // 初期强度的对手；低等级仍保留轻松局和追赶局的随机性。
  int tierBoost = base >= 100 ? 2 : base >= 50 ? 1 : 0;
  int level = base + delta + ((luckRoll >= 70) ? tierBoost : 0);
  if (level < 1) level = 1;
...
BattleStats wildBattleStats(int16_t dex, uint8_t level) {
  ...
  BattleStats stats = {};
  uint8_t rankGrowth = (uint8_t)(lvl / 10 + lvl / 25);
  stats.hp = (uint16_t)entry.bHp + (uint16_t)lvl * 2 + (uint16_t)rankGrowth * 5;
  stats.atk = (uint16_t)entry.bAtk + lvl + rankGrowth * 2;
  stats.def = (uint16_t)entry.bDef + lvl + rankGrowth * 2;
  stats.spe = (uint16_t)entry.bSpe + lvl + rankGrowth;
  stats.level = lvl;
