...
static const char *const UPDATE_BIN_URL = "https://raw.githubusercontent.com/socquique/TamaPoke/main/web/firmware/tamapoke-app.bin";
...
void onSwipeV(int dir) {
  ...
  if (dir < 0) {  // 向上滑动：返回当前页面的上一级
    ...
    if (kbOpen) { kbOpen = false; cardOpen = true; return; }
...
void renderClock() {
  ...
  gfx->setTextColor(UI_INK); gfx->setTextSize(1); gfx->setCursor(56, 164); gfx->print("屏幕亮度");
  gfx->fillRoundRect(154, 168, 252, 12, 5, UI_TRACK);
  int knob = 156 + (int)brightnessSetting * 246 / 255;
  gfx->fillRoundRect(156, 170, knob - 156, 8, 4, C565(0x3c,0x78,0xc2));
  gfx->fillCircle(knob, 174, 9, C565(0x3c,0x78,0xc2));
  gfx->setCursor(56, 210); gfx->print("音量");
  const char *vnames[3] = { "小", "中", "大" };
  for (uint8_t i = 0; i < 3; i++) {
    int vx = 150 + i * 70;
    gfx->fillRoundRect(vx, 194, 56, 36, 8, volumeLevel == i ? C565(0x3c,0x78,0xc2) : UI_WHITE);
    gfx->drawRoundRect(vx, 194, 56, 36, 8, UI_INK);
    gfx->setTextColor(volumeLevel == i ? UI_WHITE : UI_INK); gfx->setTextSize(1);
    gfx->setCursor(vx + (56 - uiTextWidth(vnames[i], 6)) / 2, 200); gfx->print(vnames[i]);
  }

  gfx->setTextColor(UI_INK); gfx->setTextSize(1); gfx->setCursor(56, 256); gfx->print("触摸");
  const char *touchNames[3] = { "高", "中", "低" };
  uint8_t touchChoice = touchSensitivity <= 43 ? 0 : (touchSensitivity >= 68 ? 2 : 1);
  for (uint8_t i = 0; i < 3; i++) {
    int tx = 150 + i * 70;
    gfx->fillRoundRect(tx, 240, 56, 36, 8, touchChoice == i ? C565(0x3c,0x78,0xc2) : UI_WHITE);
    gfx->drawRoundRect(tx, 240, 56, 36, 8, UI_INK);
    gfx->setTextColor(touchChoice == i ? UI_WHITE : UI_INK); gfx->setTextSize(1);
    gfx->setCursor(tx + (56 - uiTextWidth(touchNames[i], 6)) / 2, 243); gfx->print(touchNames[i]);
  }
  ...
  gfx->fillRoundRect(286, 408, 116, 34, 9, C565(0x3c,0x78,0xc2));
  gfx->setTextColor(UI_WHITE);
  gfx->setTextSize(1);
  gfx->setCursor(286 + (116 - uiTextWidth("更新", 6)) / 2, 418);
  gfx->print("更新");

  gfx->setTextColor(UI_TRACK);
  gfx->setTextSize(1);
  if (wifiNoticeUntil && millis() < wifiNoticeUntil) {
    gfx->setCursor(CX - uiTextWidth(wifiNotice, 6) / 2, 394);
    gfx->print(wifiNotice);
  }
  // 版本号固定在设置页底部，更新按钮位于其上方。
  char ver[20];
  snprintf(ver, sizeof(ver), "v%s", FW_VERSION);
  gfx->setCursor(82, 420);
  gfx->print(ver);
...
void clockTap(int16_t x, int16_t y) {
  ...
  if (y >= 160 && y <= 186 && x >= 144 && x <= 414) {
    brightnessSetting = (uint8_t)constrain((x - 144) * 255 / 272, 20, 255);
    panel->setBrightness(brightnessSetting);
  ...
  if (y >= 346 && y <= 448) {
    ...
    if (x >= 278 && x <= 410 && y >= 404) { onlineUpdate(); return; }
  }
}
...
void renderKeyboard() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - uiTextWidth(T(S_NAME), 6) / 2, 56);
  gfx->print(T(S_NAME));
  // buffer actual
