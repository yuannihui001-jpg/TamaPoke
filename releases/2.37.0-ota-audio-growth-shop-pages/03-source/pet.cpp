...
// 体重随着等级自然增长，并缓慢回到当前成长阶段的健康区间。食物仍会
// 让体重短期超过目标，训练也仍然可以额外消耗体重。
static void adjustNaturalWeight(Pet &pet) {
  uint16_t target = 8 + min((uint8_t)30, pet.level()) * 2;
  if (pet.speciesId > 0) target += (uint8_t)(pet.speciesId % 6);
  if (target > 80) target = 80;
  if (pet.weight < target) pet.weight++;
  else if (pet.weight > target) pet.weight--;
}
...
void Pet::syncClock(uint32_t nowEpoch) {
  ...
  for (uint32_t i = 0; i < mins; i++) {
    ...
    if (ageMinutes % 3 == 0) adjustNaturalWeight(*this);
