...
static const uint8_t WIFI_NETWORKS_PER_PAGE = 6;
static const uint8_t WIFI_NETWORK_CAPACITY = 24;
uint8_t wifiNetworkCount = 0;
uint8_t wifiListPage = 0;
char wifiSsids[WIFI_NETWORK_CAPACITY][33] = {};
...
uint8_t equipmentPickerPage = 0;
// 装备槽围绕大号宠物排布，避开圆屏边缘并对应头盔、护甲、手部和鞋子。
static const int EQUIP_BOX_X[5] = { 170, 18, 170, 18, 322 };
static const int EQUIP_BOX_Y[5] = { 58, 166, 334, 258, 258 };
...
uint8_t touchVolumeLevel = 1;
uint8_t touchSensitivity = 55; // gesture threshold in pixels; lower is more sensitive
bool wifiConnecting = false;
bool otaCheckQueued = false;
bool otaCheckRunning = false;
bool otaDownloadQueued = false;
bool otaDownloadRunning = false;
uint32_t wifiConnectStarted = 0;
uint32_t onlineUpdateReadyAt = 0;
uint32_t otaDownloadReadyAt = 0;
...
bool warehouseDetailOpen = false;
uint8_t warehouseDetailSlot = 0;
...
void warehouseDetailTap(int16_t x, int16_t y);
void serviceWifi();
...
void onlineUpdate();
void performOnlineVersionCheck();
...
static uint8_t collectDecorPropSlots(uint8_t *slots);
static uint8_t equipmentUiSlotForItem(uint8_t item);
static uint8_t collectEquipmentChoices(uint8_t equipSlot, uint8_t *items);
void equipmentPickerTap(int16_t x, int16_t y);
...
void setup() {
  ...
  touchVolumeLevel = min((uint8_t)2, uiPrefs.getUChar("tapvol", 1));
  ...
  audioSetVolume(volumeLevel);
  audioSetTouchVolume(touchVolumeLevel);
...
// 扫描附近的可见网络。每页显示 6 个，更多结果通过左右滑动翻页；
// 密码输入沿用同一套触摸键盘。
void openWifiPicker() {
  wifiPickerOpen = true;
  wifiSelected = -1;
  ...
  if (n > 0) {
    for (int i = 0; i < n && wifiNetworkCount < WIFI_NETWORK_CAPACITY; ++i) {
      String ssid = WiFi.SSID(i);
      ssid.trim();
      if (!ssid.length()) continue;
      bool duplicate = false;
      for (uint8_t saved = 0; saved < wifiNetworkCount; ++saved) {
        if (ssid.equals(wifiSsids[saved])) {
          duplicate = true;
          break;
        }
      }
      if (duplicate) continue;
      strncpy(wifiSsids[wifiNetworkCount], ssid.c_str(), sizeof(wifiSsids[0]) - 1);
      wifiSsids[wifiNetworkCount][sizeof(wifiSsids[0]) - 1] = 0;
      ++wifiNetworkCount;
    }
  }
  WiFi.scanDelete();
...
void wifiPickerTap(int16_t x, int16_t y) {
  uint8_t first = wifiListPage * WIFI_NETWORKS_PER_PAGE;
  uint8_t visible = first < wifiNetworkCount
                        ? min((uint8_t)(wifiNetworkCount - first), WIFI_NETWORKS_PER_PAGE)
                        : 0;
  if (y >= 92 && y < 92 + (int)visible * 42 && x >= 58 && x <= 408) {
    int row = (y - 92) / 42;
    int index = first + row;
    if (row >= 0 && row < visible && index < wifiNetworkCount) openWifiPassword((int8_t)index);
    return;
  }
  if (y >= 370 && y <= 428) {
...
void renderWifiPicker() {
  ...
  uint8_t pages = max((uint8_t)1,
                      (uint8_t)((wifiNetworkCount + WIFI_NETWORKS_PER_PAGE - 1) /
                                WIFI_NETWORKS_PER_PAGE));
  if (wifiListPage >= pages) wifiListPage = pages - 1;
  uint8_t first = wifiListPage * WIFI_NETWORKS_PER_PAGE;
  uint8_t visible = first < wifiNetworkCount
                        ? min((uint8_t)(wifiNetworkCount - first), WIFI_NETWORKS_PER_PAGE)
                        : 0;
  ...
  if (pages > 1) {
    int dotsWidth = pages * 12 - 6;
    int dotsX = CX - dotsWidth / 2;
    for (uint8_t page = 0; page < pages; ++page) {
      gfx->fillCircle(dotsX + page * 12, 363, page == wifiListPage ? 4 : 2,
                      page == wifiListPage ? 0x3BD8 : UI_TRACK);
    }
  }
...
void serviceWifi() {
  if (!wifiConnecting) {
    if (otaDownloadQueued && WiFi.status() == WL_CONNECTED &&
        (int32_t)(millis() - otaDownloadReadyAt) >= 0 && !otaDownloadRunning) {
      otaDownloadQueued = false;
      otaDownloadRunning = true;
      performOnlineUpdate();
      otaDownloadRunning = false;
      return;
    }
    if (otaCheckQueued && WiFi.status() == WL_CONNECTED &&
        (int32_t)(millis() - onlineUpdateReadyAt) >= 0) {
      otaCheckQueued = false;
      otaCheckRunning = true;
      performOnlineVersionCheck();
  ...
  if (status == WL_CONNECTED) {
    ...
    bool checkAfterConnect = otaCheckQueued;
    if (checkAfterConnect) {
      // DHCP 完成后给 DNS/TLS 留出稳定时间，再发起更新检查。不要主动
      // 断开或重连 WiFi，否则用户会看到开关自行关闭又打开。
      onlineUpdateReadyAt = millis() + 900;
      setWifiNotice("网络已连接，正在检查", 12000);
    } else {
...
class WifiTransferAwake {
 public:
  WifiTransferAwake() { WiFi.setSleep(false); delay(30); }
...
void onlineUpdate() {
  if (otaCheckQueued || otaCheckRunning || otaDownloadQueued || otaDownloadRunning) {
    setWifiNotice("更新任务正在进行", 6000);
    return;
  }
  otaCheckQueued = true;
  onlineUpdateReadyAt = millis() + 300;
  setWifiNotice("正在检查更新", 20000);
  if (WiFi.status() != WL_CONNECTED) {
    if (wifiConnecting) {
      setWifiNotice("正在连接 WiFi", 15000);
      return;
    }
    if (uiPrefs.getString("ssid", "").length()) {
      startWifiConnection();
      setWifiNotice("正在连接 WiFi", 15000);
    } else {
      otaCheckQueued = false;
      setWifiNotice("请先设置 WiFi");
    }
    return;
  }
}
...
void performOnlineVersionCheck() {
  ...
  vstart += 11;
  int vend = checkResponse.indexOf('"', vstart);
  if (vend <= vstart) {
    setWifiNotice("更新版本无效");
    return;
  }
  String remote = checkResponse.substring(vstart, vend);
  remote.toCharArray(updateVersion, sizeof(updateVersion));
  updateAvailable = remote != String(FW_VERSION);
...
void updateDialogTap(int16_t x, int16_t y) {
  ...
  if (y < 320 || y > 410) return;
  if (x >= 94 && x < 220) {
    updateDialogOpen = false;
    return;
  }
  if (x >= 246 && x < 372 && updateAvailable) {
    updateDialogOpen = false;
    otaDownloadQueued = true;
    otaDownloadReadyAt = millis() + 300;
    setWifiNotice("准备下载更新", 30000);
  }
}
...
void onSwipeV(int dir) {
  ...
  if (dir < 0) {  // 向上滑动：返回当前页面的上一级
    ...
    if (clockOpen) { applyClock(); return; }
    if (shopOpen) {
    ...
    if (warehouseOpen) {
      if (warehouseDetailOpen) { warehouseDetailOpen = false; return; }
      warehouseOpen = false;
      worldOpen = true;
      return;
    }
    ...
    if (cardOpen) {
      if (cardPage == 4 && equipmentActionOpen) { equipmentActionOpen = false; return; }
      cardOpen = false;
      return;
    }
...
void onSwipe(int dir) {
  ...
  if (wifiPickerOpen) {
    uint8_t pages = max((uint8_t)1,
                        (uint8_t)((wifiNetworkCount + WIFI_NETWORKS_PER_PAGE - 1) /
                                  WIFI_NETWORKS_PER_PAGE));
    int next = (int)wifiListPage + (dir < 0 ? 1 : -1);
    if (next < 0) next = 0;
    if (next >= pages) next = pages - 1;
    if (next != wifiListPage) {
      wifiListPage = (uint8_t)next;
      sfxPlay(SFX_TAP);
    }
    return;
  }
  ...
  if (decorOpen) {
    uint8_t propSlots[SHOP_ITEMS_PER_CATEGORY];
    uint8_t total = (uint8_t)(8 + collectDecorPropSlots(propSlots));
    uint8_t pages = (uint8_t)((total + 7) / 8);
    if (!pages) pages = 1;
    int next = (int)decorPage + (dir < 0 ? 1 : -1);
    if (next >= 0 && next < pages) decorPage = (uint8_t)next;
    return;
  }
  if (gameOpen || memoryOpen || gameMenuOpen || worldOpen || warehouseOpen || kbOpen || clockOpen || battleOpen || feedMenuUntil || toyMenuUntil) return;
  ...
  if (shopOpen) {
    ...
    if (shopFromGallery && dir > 0) {
      ...
      int next = (int)shopScroll + (dir < 0 ? pageSize : -pageSize);
      int maxScroll = SHOP_ITEMS_PER_CATEGORY > 5
                          ? ((SHOP_ITEMS_PER_CATEGORY - 1) / pageSize) * pageSize
                          : 0;
      if (next < 0) next = 0;
  ...
  if (cardOpen) {  // dentro de la ficha: cambiar entre las 5 paginas
    if (cardPage == 4 && equipmentActionOpen) {
      uint8_t choices[SHOP_ITEMS_PER_CATEGORY];
      uint8_t count = collectEquipmentChoices(equipmentActionSlot, choices);
      uint8_t pages = (uint8_t)max(1, (int)((count + 5) / 6));
      int next = (int)equipmentPickerPage + (dir < 0 ? 1 : -1);
      if (next >= 0 && next < pages) equipmentPickerPage = (uint8_t)next;
      return;
    }
...
void onTap(int16_t x, int16_t y) {
  ...
  // 每次有效轻触都立即给声音反馈；audio.cpp 会过滤同一次操作里重复的 TAP。
  sfxPlay(SFX_TAP);
...
int sceneHour() {
  ...
  // The RTC is edited and stored as local Beijing time, so no extra offset is
  // applied here. Adding another UTC+8 would move every scene eight hours.
...
static void drawRoomDetails(uint8_t biome, uint32_t now, bool night) {
  ...
  // 每件道具使用符合实物用途的场景锚点：树木、房屋和风车贴近地平线，
  // 家具与玩具落在地面。旧版四列网格把前两排画在天空中，看起来像
  // 错位；这里按物品类型定位，并由后续宠物绘制层自然遮挡道具。
  static const int PROP_X[20] = {
    76, 410, 96, 372, 233, 108, 112, 354, 410, 233,
  ...
  for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; ++slot)
    if (props & (uint32_t)(1UL << slot))
      drawPropProduct(slot, PROP_X[slot], PROP_Y[slot], PROP_SCALE[slot]);
  (void)biome;
  (void)now;
}
...
void render() {
  ...
  if (pet.isEgg()) {
    ...
    // status bars and controls remain over the same full-screen scene background
    // so the circular home view has no abrupt white strip at the bottom.
    drawBars();
    drawButtons();
    drawCelebration();
...
void drawWorldBadge() {
  drawTopCoins(176, 90, gNight ? UI_WHITE : UI_INK);
}
...
void decorTap(int16_t x, int16_t y) {
  ...
  uint8_t propCount = 0;
  for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; ++slot)
    if (pet.warehouseCount(SHOP_CAT_PROP, slot)) propSlots[propCount++] = slot;
  uint8_t total = (uint8_t)(8 + propCount);
  uint8_t pages = (uint8_t)((total + 7) / 8);
  if (!pages) pages = 1;
  if (y >= 350 && y <= 420 && pages > 1) {
    if (x < 180 && decorPage > 0) decorPage--;
    else if (x > 286 && decorPage + 1 < pages) decorPage++;
    return;
  }
  if (x < 55 || x > 411 || y < 92 || y >= 92 + 4 * 64) return;
  uint8_t col = x >= 233 ? 1 : 0;
  uint8_t row = (uint8_t)((y - 92) / 64);
  if (row > 3) return;
  int cardX = 55 + col * 184;
  if (x > cardX + 172 || y >= 92 + row * 64 + 52) return;
  uint8_t entry = (uint8_t)(decorPage * 8 + row * 2 + col);
  ...
  if (entry < 8) {
    ...
    if (pet.decorOwned & (1u << slot)) {
      ...
      // Locked legacy decor remains a shortcut to the prop shop.
      decorOpen = false;
      shopOpen = true;
...
static uint8_t collectDecorPropSlots(uint8_t *slots) {
  uint8_t count = 0;
  for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; ++slot)
    if (pet.warehouseCount(SHOP_CAT_PROP, slot)) slots[count++] = slot;
  return count;
}
...
void renderDecor() {
  ...
  gfx->setCursor(CX - uiTextWidth(T(S_DECORATE), gLang == LANG_ZH ? 6 : 9) / 2, 46);
  gfx->print(T(S_DECORATE));

  uint8_t propSlots[SHOP_ITEMS_PER_CATEGORY];
  uint8_t propCount = collectDecorPropSlots(propSlots);
  uint8_t total = (uint8_t)(8 + propCount);
  uint8_t pages = (uint8_t)((total + 7) / 8);
  if (!pages) pages = 1;
  if (decorPage >= pages) decorPage = pages - 1;
  const char *labels[8] = { T(S_BALL), T(S_FLOWERS), T(S_TENT), T(S_LAMP), T(S_DRUM), T(S_BLOCKS), T(S_TRAIN), T(S_KITE) };
  for (uint8_t i = 0; i < 8; ++i) {
    uint8_t entry = (uint8_t)(decorPage * 8 + i);
    if (entry >= total) continue;
    int x = 55 + (i % 2) * 184;
    int y = 92 + (i / 2) * 64;
    gfx->fillRoundRect(x, y, 172, 52, 9, C565(0x2f, 0x42, 0x58));
    gfx->drawRoundRect(x, y, 172, 52, 9, UI_WHITE);
    gfx->setTextColor(UI_WHITE);
    gfx->setTextSize(1);
    if (entry < 8) {
      uint8_t slot = entry;
      ...
      drawDecorIcon(slot, x + 18, y + 26, gNight);
      String name = wifiLabelFit(String(labels[slot]), 66);
      gfx->setCursor(x + 38, y + 7); gfx->print(name.c_str());
      gfx->setTextColor(owned ? (placed ? UI_BAR_OK : UI_WHITE) : C565(0xa8, 0xb0, 0xc0));
      gfx->setCursor(x + 38, y + 29); gfx->print(owned ? (placed ? "已布置" : "点击") : "未拥有");
      if (!owned) drawLockBadge(x + 154, y + 16);
    } else {
      uint8_t slot = propSlots[entry - 8];
      bool placed = (pet.roomProps() & (uint32_t)(1UL << slot)) != 0;
      drawPropProduct(slot, x + 18, y + 26, 0.30f);
      String name = wifiLabelFit(String(SHOP_PRODUCTS[SHOP_CAT_PROP][slot].name), 40);
      gfx->setTextColor(UI_WHITE);
      gfx->setCursor(x + 38, y + 7); gfx->print(name.c_str());
      const int bx[2] = { x + 80, x + 125 };
      const char *bl[2] = { "布置", "收回" };
      const uint16_t bc[2] = { UI_BAR_OK, UI_BAR_BAD };
      for (uint8_t b = 0; b < 2; ++b) {
        gfx->fillRoundRect(bx[b], y + 23, 42, 26, 6, placed && b == 0 ? C565(0xc8, 0xd9, 0xcd) : UI_WHITE);
        gfx->drawRoundRect(bx[b], y + 23, 42, 26, 6, UI_INK);
        gfx->setTextColor(bc[b]);
        gfx->setCursor(bx[b] + (42 - uiTextWidth(bl[b], 6)) / 2, y + 28); gfx->print(bl[b]);
      }
    }
  }
  ...
  gfx->setTextSize(1);
  if (pages > 1) {
    gfx->drawLine(100, 384, 124, 384, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(100, 384, 112, 374, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(100, 384, 112, 394, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(342, 384, 366, 384, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(366, 384, 354, 374, gNight ? UI_WHITE : UI_INK);
    gfx->drawLine(366, 384, 354, 394, gNight ? UI_WHITE : UI_INK);
  }
  char pageLabel[12]; snprintf(pageLabel, sizeof(pageLabel), "%u/%u", (unsigned)(decorPage + 1), (unsigned)pages);
  gfx->setCursor(CX - uiTextWidth(pageLabel, 6) / 2, 378); gfx->print(pageLabel);
  gfx->setCursor(CX - uiTextWidth(T(S_TAP_DECOR), 6) / 2, 422);
...
static void drawProductIcon(uint8_t category, uint8_t slot, int cx, int cy, uint8_t s) {
  if (category >= SHOP_CAT_COUNT || slot >= SHOP_ITEMS_PER_CATEGORY) return;
  const uint8_t productId = shopProductId(category, slot);
  const uint8_t pixelScale = s == 1 ? 1 : 3;
  const int side = SHOP_ICON_SIZE * pixelScale;
  const int x0 = cx - side / 2;
  const int y0 = cy - side / 2;
  for (uint8_t y = 0; y < SHOP_ICON_SIZE; ++y) {
    for (uint8_t x = 0; x < SHOP_ICON_SIZE; ++x) {
      const uint16_t pixel = (uint16_t)y * SHOP_ICON_SIZE + x;
      const uint8_t packed = pgm_read_byte(&SHOP_ICON_PIXELS[productId][pixel >> 1]);
      const uint8_t paletteIndex = (pixel & 1) ? (packed & 0x0F) : (packed >> 4);
      if (!paletteIndex) continue;
      const uint16_t color = pgm_read_word(&SHOP_ICON_PALETTES[productId][paletteIndex]);
      if (pixelScale == 1) gfx->drawPixel(x0 + x, y0 + y, color);
      else gfx->fillRect(x0 + x * pixelScale, y0 + y * pixelScale, pixelScale, pixelScale, color);
    }
  }
}
...
static bool warehouseEquipmentVisible(uint8_t slot) {
  if (pet.warehouseCount(SHOP_CAT_EQUIPMENT, slot)) return true;
  for (uint8_t i = 0; i < EQUIP_SLOT_COUNT; ++i)
    if (pet.equippedItem(i) == slot) return true;
  return false;
}

static bool warehouseItemVisible(uint8_t category, uint8_t slot) {
  if (category == SHOP_CAT_EQUIPMENT) return warehouseEquipmentVisible(slot);
  if (category == SHOP_CAT_FOOD && slot < 4) return true;
  return pet.warehouseCount(category, slot) != 0;
}

static bool warehouseEquipmentEquipped(uint8_t item, uint8_t *equipSlot = nullptr) {
  for (uint8_t slot = 0; slot < EQUIP_SLOT_COUNT; ++slot) {
    if (pet.equippedItem(slot) != item) continue;
...
void warehouseDetailTap(int16_t x, int16_t y) {
  if (!warehouseDetailOpen || y < 326 || y > 390) return;
  uint8_t slot = warehouseDetailSlot;
  if (shopPage == SHOP_CAT_EQUIPMENT) {
    if (x >= 72 && x <= 146) {
      warehouseDetailOpen = false;
    } else if (x >= 152 && x <= 226) {
      bool already = warehouseEquipmentEquipped(slot);
      bool ok = !already && pet.equipShopProduct(slot);
      warehouseDetailOpen = false;
      economyNoticeText(already ? "已经装备" : (ok ? "装备完成" : "无法装备"), already || ok);
    } else if (x >= 232 && x <= 306) {
      uint8_t equipSlot = 0;
      bool equipped = warehouseEquipmentEquipped(slot, &equipSlot);
      bool ok = equipped && pet.unequipSlot(equipSlot);
      warehouseDetailOpen = false;
      economyNoticeText(ok ? "卸下完成" : "尚未装备", ok);
    } else if (x >= 312 && x <= 386) {
      bool ok = pet.sellShopProduct(shopPage, slot);
      warehouseDetailOpen = false;
      economyNoticeText(ok ? "出售完成" : "无法出售", ok);
    }
    return;
  }
  ...
  if (x >= 185 && x <= 281) {
    ...
    if (shopPage == SHOP_CAT_PROP) {
      bool placed = (pet.roomProps() & (uint32_t)(1UL << slot)) != 0;
      ok = placed || pet.toggleProp(slot);
      economyNoticeText(placed ? "已经布置" : (ok ? "已布置" : "无法布置"), ok);
    } else if (shopPage == SHOP_CAT_TRAVEL) {
      if (pet.poopFuelBatchCount() == 0) {
        economyNoticeText("燃料不足", false);
      } else if (pet.useShopProduct(shopPage, slot)) {
        pet.joy = pet.energy = pet.health = 100;
        travelSlot = slot;
        travelUntil = millis() + 10000;
        travelOpen = true;
        ok = true;
      } else economyNoticeText("无法出发", false);
    } else {
      ok = pet.useShopProduct(shopPage, slot);
      economyNoticeText(ok ? "使用完成" : "无法使用", ok);
    }
    warehouseDetailOpen = false;
    return;
  }
  if (x >= 300 && x <= 396) {
    ...
    if (shopPage == SHOP_CAT_PROP) {
      bool placed = (pet.roomProps() & (uint32_t)(1UL << slot)) != 0;
      ok = !placed || pet.toggleProp(slot);
      economyNoticeText(placed ? (ok ? "已收回" : "无法收回") : "已经收回", ok);
    } else {
      ok = pet.sellShopProduct(shopPage, slot);
      economyNoticeText(ok ? "出售完成" : "无法出售", ok);
    }
    warehouseDetailOpen = false;
  }
...
void warehouseTap(int16_t x, int16_t y) {
  if (warehouseDetailOpen) {
    warehouseDetailTap(x, y);
    return;
  }
  if (y >= 88 && y < 172) {
  ...
  if (y >= 405 && y <= 438) {
    ...
    for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; ++slot)
      if (warehouseItemVisible(shopPage, slot)) ownedCount++;
    uint8_t perPage = shopPage == SHOP_CAT_EQUIPMENT ? 5 : 12;
    uint8_t pages = (uint8_t)max(1, (int)((ownedCount + perPage - 1) / perPage));
    if (x < 160 && warehouseScroll > 0) warehouseScroll--;
    else if (x > 306 && warehouseScroll + 1 < pages) warehouseScroll++;
    return;
  }
  if (shopPage == SHOP_CAT_EQUIPMENT && y >= 174 && y < 414) {
    uint8_t row = (uint8_t)((y - 174) / 48);
    uint8_t target = (uint8_t)(warehouseScroll * 5 + row);
    uint8_t seen = 0;
    for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; ++slot) {
      if (!warehouseEquipmentVisible(slot)) continue;
      if (seen++ != target) continue;
      if (x >= 58 && x <= 408) {
        warehouseDetailSlot = slot;
        warehouseDetailOpen = true;
        sfxPlay(SFX_TAP);
      }
      return;
    }
  ...
  if (y >= 180 && y < 414) {
    ...
    uint8_t seen = 0;
    for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; slot++) {
      if (!warehouseItemVisible(shopPage, slot)) continue;
      if (seen == (uint8_t)(warehouseScroll * 12 + row * 2 + col)) {
        warehouseDetailSlot = slot;
        warehouseDetailOpen = true;
        sfxPlay(SFX_TAP);
        return;
...
static void renderWarehouseDetail() {
  if (!warehouseDetailOpen || warehouseDetailSlot >= SHOP_ITEMS_PER_CATEGORY) return;
  const uint8_t category = shopPage;
  const uint8_t slot = warehouseDetailSlot;
  const ShopProduct &product = SHOP_PRODUCTS[category][slot];
  gfx->fillRoundRect(64, 74, 338, 306, 18, UI_WHITE);
  gfx->drawRoundRect(64, 74, 338, 306, 18, UI_INK);
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(1);
  gfx->setCursor(CX - uiTextWidth(product.name, 6) / 2, 88);
  gfx->print(product.name);
  drawProductIcon(category, slot, CX, 160, 2);

  char detail1[48] = "";
  char detail2[48] = "";
  if (category == SHOP_CAT_FOOD) {
    snprintf(detail1, sizeof(detail1), "饥饿感 +%u", (unsigned)product.value);
    snprintf(detail2, sizeof(detail2), "喜悦感 +%u", (unsigned)(product.value / 3));
  } else if (category == SHOP_CAT_TOY) {
    snprintf(detail1, sizeof(detail1), "喜悦感 +%u", (unsigned)product.value);
    strcpy(detail2, "陪宠物玩耍使用");
  } else if (category == SHOP_CAT_MEDICINE) {
    if (slot == 1 || slot == 8) snprintf(detail1, sizeof(detail1), "清洁状态 +%u", (unsigned)product.value);
    else if (slot == 6) snprintf(detail1, sizeof(detail1), "能量状态 +%u", (unsigned)product.value);
    else snprintf(detail1, sizeof(detail1), "健康状态 +%u", (unsigned)product.value);
    strcpy(detail2, "根据症状选择药品");
  } else if (category == SHOP_CAT_EQUIPMENT) {
    if (slot == 0 || slot == 6 || slot == 7 || slot == 10 || slot == 12 || slot == 15 || slot == 19)
      snprintf(detail1, sizeof(detail1), "攻击力 +%u", (unsigned)product.value);
    else if (slot == 1 || slot == 2 || slot == 3 || slot == 8 || slot == 11 || slot == 16 || slot == 17)
      snprintf(detail1, sizeof(detail1), "防御力 +%u", (unsigned)product.value);
    else snprintf(detail1, sizeof(detail1), "免疫力 +%u", (unsigned)product.value);
    strcpy(detail2, "装备后持续生效");
  } else if (category == SHOP_CAT_PROP) {
    strcpy(detail1, "可布置到当前场景");
    strcpy(detail2, (pet.roomProps() & (uint32_t)(1UL << slot)) ? "当前已经布置" : "当前放在仓库");
  } else {
    strcpy(detail1, "需要燃料 30");
    strcpy(detail2, "旅行后状态全部恢复");
  }
  gfx->setCursor(CX - uiTextWidth(detail1, 6) / 2, 202); gfx->print(detail1);
  gfx->setCursor(CX - uiTextWidth(detail2, 6) / 2, 234); gfx->print(detail2);
  char quantity[32];
  if (category == SHOP_CAT_FOOD && slot < 4) strcpy(quantity, "数量 无限");
  else snprintf(quantity, sizeof(quantity), "数量 %u", (unsigned)pet.warehouseCount(category, slot));
  gfx->setCursor(CX - uiTextWidth(quantity, 6) / 2, 270); gfx->print(quantity);

  if (category == SHOP_CAT_EQUIPMENT) {
    const int bx[4] = { 72, 152, 232, 312 };
    const char *labels[4] = { "关闭", "装备", "卸下", "出售" };
    const uint16_t colors[4] = { C565(0xe0,0xe7,0xee), C565(0x55,0xb3,0x83), C565(0xe0,0xb0,0x64), C565(0xd1,0x75,0x75) };
    for (uint8_t i = 0; i < 4; ++i) {
      gfx->fillRoundRect(bx[i], 326, 74, 40, 9, colors[i]);
      gfx->drawRoundRect(bx[i], 326, 74, 40, 9, UI_INK);
      gfx->setTextColor(i == 0 ? UI_INK : UI_WHITE);
      gfx->setCursor(bx[i] + (74 - uiTextWidth(labels[i], 6)) / 2, 333);
      gfx->print(labels[i]);
    }
  } else {
    const int bx[3] = { 70, 185, 300 };
    const char *middle = category == SHOP_CAT_TRAVEL ? "出发" : category == SHOP_CAT_PROP ? "布置" : "使用";
    const char *right = category == SHOP_CAT_PROP ? "收回" : "出售";
    const char *labels[3] = { "关闭", middle, right };
    const uint16_t colors[3] = { C565(0xe0,0xe7,0xee), C565(0x55,0xb3,0x83), C565(0xd1,0x75,0x75) };
    for (uint8_t i = 0; i < 3; ++i) {
      gfx->fillRoundRect(bx[i], 326, 96, 40, 9, colors[i]);
      gfx->drawRoundRect(bx[i], 326, 96, 40, 9, UI_INK);
      gfx->setTextColor(i == 0 ? UI_INK : UI_WHITE);
      gfx->setCursor(bx[i] + (96 - uiTextWidth(labels[i], 6)) / 2, 333);
      gfx->print(labels[i]);
    }
  }
}
...
void renderWarehouse() {
  ...
  bool equipmentPage = shopPage == SHOP_CAT_EQUIPMENT;
  uint8_t ownedCount = 0;
  for (uint8_t slot = 0; slot < SHOP_ITEMS_PER_CATEGORY; slot++)
    if (warehouseItemVisible(shopPage, slot)) ownedCount++;
  uint8_t pageCount = equipmentPage ? (ownedCount + 4) / 5 : (ownedCount + 11) / 12;
  if (!pageCount) pageCount = 1;
  if (warehouseScroll >= pageCount) warehouseScroll = pageCount - 1;
  uint8_t first = warehouseScroll * (equipmentPage ? 5 : 12);
  if (equipmentPage) {
    for (uint8_t row = 0; row < 5; ++row) {
      uint8_t slot = 0, seen = 0;
      bool found = false;
      for (; slot < SHOP_ITEMS_PER_CATEGORY; ++slot) {
        if (!warehouseEquipmentVisible(slot)) continue;
        if (seen++ == first + row) { found = true; break; }
      }
      if (!found) continue;
      int x = 58, y = 174 + row * 48;
      gfx->fillRoundRect(x, y, 350, 44, 8, C565(0xe6, 0xea, 0xef));
      drawProductIcon(shopPage, slot, x + 22, y + 22, 1);
      String itemName = wifiLabelFit(String(SHOP_PRODUCTS[shopPage][slot].name), 210);
      gfx->setTextColor(UI_INK); gfx->setTextSize(1);
      gfx->setCursor(x + 44, y + 9); gfx->print(itemName.c_str());
      char qty[12]; snprintf(qty, sizeof(qty), "x%u", (unsigned)pet.warehouseCount(shopPage, slot));
      gfx->setCursor(370 - uiTextWidth(qty, 6), y + 9); gfx->print(qty);
      if (warehouseEquipmentEquipped(slot)) gfx->fillCircle(392, y + 22, 6, UI_BAR_OK);
    }
  } else for (uint8_t row = 0; row < 6; row++) {
    for (uint8_t col = 0; col < 2; col++) {
      ...
      char qty[12];
      if (shopPage == SHOP_CAT_FOOD && slot < 4) strcpy(qty, "无限");
      else snprintf(qty, sizeof(qty), "x%u", (unsigned)pet.warehouseCount(shopPage, slot));
      int qtyWidth = uiTextWidth(qty, 6);
      String itemName = wifiLabelFit(String(SHOP_PRODUCTS[shopPage][slot].name), 134 - qtyWidth);
      gfx->print(itemName.c_str());
      gfx->setCursor(x + 186 - uiTextWidth(qty, 6), y + 6); gfx->print(qty);
    }
  }
  gfx->setTextSize(1);
  char pg[16]; snprintf(pg, sizeof(pg), "%u/%u", (unsigned)(warehouseScroll + 1), (unsigned)pageCount);
  gfx->setCursor(CX - uiTextWidth(pg, 6) / 2, 420); gfx->print(pg);
  if (pageCount > 1) {
...
void renderClock() {
  ...
  const int volumeRowY = 170;
  gfx->setCursor(42, volumeRowY + (36 - CN_GLYPH_HEIGHT) / 2); gfx->print("音量");
  const char *vnames[3] = { "小", "中", "大" };
  for (uint8_t i = 0; i < 3; i++) {
    int vx = 146 + i * 72;
    gfx->fillRoundRect(vx, volumeRowY, 60, 36, 8, volumeLevel == i ? C565(0x3c,0x78,0xc2) : UI_WHITE);
    gfx->drawRoundRect(vx, volumeRowY, 60, 36, 8, UI_INK);
    gfx->setTextColor(volumeLevel == i ? UI_WHITE : UI_INK); gfx->setTextSize(1);
    gfx->setCursor(vx + (60 - uiTextWidth(vnames[i], 6)) / 2,
                   volumeRowY + (36 - CN_GLYPH_HEIGHT) / 2); gfx->print(vnames[i]);
  }

  const int touchRowY = 214;
  gfx->setTextColor(UI_INK); gfx->setTextSize(1);
  gfx->setCursor(42, touchRowY + (36 - CN_GLYPH_HEIGHT) / 2); gfx->print("触摸");
  const char *touchNames[3] = { "高", "中", "低" };
  uint8_t touchChoice = touchSensitivity <= 43 ? 0 : (touchSensitivity >= 68 ? 2 : 1);
  for (uint8_t i = 0; i < 3; i++) {
    int tx = 146 + i * 72;
    gfx->fillRoundRect(tx, touchRowY, 60, 36, 8, touchChoice == i ? C565(0x3c,0x78,0xc2) : UI_WHITE);
    gfx->drawRoundRect(tx, touchRowY, 60, 36, 8, UI_INK);
    gfx->setTextColor(touchChoice == i ? UI_WHITE : UI_INK); gfx->setTextSize(1);
    gfx->setCursor(tx + (60 - uiTextWidth(touchNames[i], 6)) / 2,
                   touchRowY + (36 - CN_GLYPH_HEIGHT) / 2); gfx->print(touchNames[i]);
  }

  const int touchVolumeRowY = 258;
  gfx->setTextColor(UI_INK); gfx->setTextSize(1);
  gfx->setCursor(42, touchVolumeRowY + (36 - CN_GLYPH_HEIGHT) / 2); gfx->print("触摸音量");
  for (uint8_t i = 0; i < 3; i++) {
    int tx = 146 + i * 72;
    gfx->fillRoundRect(tx, touchVolumeRowY, 60, 36, 8,
                       touchVolumeLevel == i ? C565(0x3c,0x78,0xc2) : UI_WHITE);
    gfx->drawRoundRect(tx, touchVolumeRowY, 60, 36, 8, UI_INK);
    gfx->setTextColor(touchVolumeLevel == i ? UI_WHITE : UI_INK); gfx->setTextSize(1);
    gfx->setCursor(tx + (60 - uiTextWidth(vnames[i], 6)) / 2,
                   touchVolumeRowY + (36 - CN_GLYPH_HEIGHT) / 2); gfx->print(vnames[i]);
  }
  ...
  gfx->setTextSize(2);
  gfx->setCursor(104 + (96 - uiTextWidth(sl, 6)) / 2, LANG_PILL_Y + 5);
  gfx->print(sl);
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(LANG_PILL_X + (LANG_PILL_W - uiTextWidth(lp, 6)) / 2, LANG_PILL_Y + 5);
  gfx->print(lp);

  bool wifiOk = WiFi.status() == WL_CONNECTED;
  gfx->fillRoundRect(58, 344, 170, 40, 10, wifiOk ? UI_BAR_OK : UI_WHITE);
  gfx->drawRoundRect(58, 344, 170, 40, 10, UI_INK);
  gfx->setTextColor(wifiOk ? UI_BG_DAY : UI_INK);
  // WiFi/SSID 中的英文缩小到原来的约 60%，避免压过中文。
  gfx->setTextSize(2);
  const char *wifiLabel = wifiOk ? "WiFi 已连" : "连接 WiFi";
  gfx->setCursor(58 + (170 - uiTextWidth(wifiLabel, 12)) / 2, 351);
  gfx->print(wifiLabel);

  gfx->fillRoundRect(238, 344, 170, 40, 10, C565(0x3c,0x78,0xc2));
  gfx->setTextColor(UI_WHITE);
  gfx->setTextSize(4);
  gfx->drawRoundRect(238, 344, 170, 40, 10, UI_INK);
  gfx->setCursor(238 + (170 - uiTextWidth("更新", 24)) / 2, 351);
  gfx->print("更新");

  // 网络连接、登记和更新结果使用居中弹窗，避免在圆屏底部被裁切。
  if (wifiNoticeUntil && millis() < wifiNoticeUntil && wifiNotice[0]) {
    String notice = wifiLabelFit(String(wifiNotice), 300, 6);
    ...
    gfx->print("提示");
    gfx->setCursor(CX - uiTextWidth(notice.c_str(), 6) / 2, 214);
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - uiTextWidth(ver, 12) / 2, 400);
  gfx->print(ver);
  ...
  if (updateDialogOpen) {
    gfx->fillRoundRect(55, 112, 356, 276, 16, UI_WHITE);
    gfx->drawRoundRect(55, 112, 356, 276, 16, UI_INK);
    gfx->setTextColor(UI_INK); gfx->setTextSize(2);
    const char *head = updateAvailable ? "发现新版本" : "已是最新版本";
    gfx->setCursor(CX - uiTextWidth(head, 6) / 2, 140); gfx->print(head);
    gfx->setTextSize(1);
    char updateInfo[40];
    if (updateAvailable) snprintf(updateInfo, sizeof(updateInfo), "当前 v%s  ->  新版 v%s", FW_VERSION, updateVersion);
    else snprintf(updateInfo, sizeof(updateInfo), "当前版本 v%s", FW_VERSION);
    gfx->setCursor(CX - uiTextWidth(updateInfo, 6) / 2, 190); gfx->print(updateInfo);
    gfx->setCursor(CX - uiTextWidth("宠物资料不会被擦除", 6) / 2, 224); gfx->print("宠物资料不会被擦除");
    gfx->fillRoundRect(94, 326, 126, 44, 10, C565(0xe0,0xe7,0xee));
    gfx->fillRoundRect(246, 326, 126, 44, 10, updateAvailable ? C565(0x55,0xb3,0x83) : UI_TRACK);
    gfx->setTextColor(UI_INK); gfx->setTextSize(1);
    gfx->setCursor(94 + (126 - uiTextWidth("取消", 6)) / 2, 340); gfx->print("取消");
    gfx->setTextColor(UI_WHITE);
    gfx->setCursor(246 + (126 - uiTextWidth("更新", 6)) / 2, 340); gfx->print("更新");
  }
  /*
  gfx->setTextSize(3);
...
void clockTap(int16_t x, int16_t y) {
  // “更新”使用比可见按钮更大的独立热区，并且优先于提示弹窗处理，
  // 避免第一次触摸只关闭旧提示、必须反复点击才有反应。
  if (y >= 340 && y <= 402 && x >= 228 && x <= 426) {
    wifiNoticeUntil = 0;
    onlineUpdate();
    return;
  }
  ...
  if (y >= 254 && y <= 298) {
    for (uint8_t i = 0; i < 3; i++) {
      if (x >= 146 + i * 72 && x < 206 + i * 72) {
        touchVolumeLevel = i;
        uiPrefs.putUChar("tapvol", touchVolumeLevel);
        audioSetTouchVolume(touchVolumeLevel);
      }
    }
    return;
...
static uint8_t equipmentUiSlotForItem(uint8_t item) {
  if (item == 2) return EQUIP_HELMET;
  if (item == 3 || item == 8 || item == 17) return EQUIP_ARMOR;
  if (item == 5) return EQUIP_SHOES;
  return (item & 1) ? EQUIP_RIGHT_HAND : EQUIP_LEFT_HAND;
}

static uint8_t collectEquipmentChoices(uint8_t equipSlot, uint8_t *items) {
  uint8_t count = 0;
  uint8_t equipped = pet.equippedItem(equipSlot);
  for (uint8_t item = 0; item < SHOP_ITEMS_PER_CATEGORY; ++item) {
    if (equipmentUiSlotForItem(item) != equipSlot) continue;
    if (!pet.warehouseCount(SHOP_CAT_EQUIPMENT, item) && equipped != item) continue;
    items[count++] = item;
  }
  return count;
}
...
void equipmentPickerTap(int16_t x, int16_t y) {
  ...
  if (y >= 334 && y <= 390) {
    ...
    if (x >= 242 && x <= 388) {
      bool ok = pet.unequipSlot(equipmentActionSlot);
      equipmentActionOpen = false;
      economyNoticeText(ok ? "卸下完成" : "尚未装备", ok);
      return;
    }
  }
  if (y < 154 || y >= 312 || x < 78 || x > 388) return;
  uint8_t col = x >= 238 ? 1 : 0;
  uint8_t row = (uint8_t)((y - 154) / 56);
  int cardX = 78 + col * 160;
  int cardY = 154 + row * 56;
  if (row > 2 || x > cardX + 150 || y > cardY + 46) return;
  uint8_t choices[SHOP_ITEMS_PER_CATEGORY];
  uint8_t count = collectEquipmentChoices(equipmentActionSlot, choices);
  uint8_t ordinal = (uint8_t)(equipmentPickerPage * 6 + row * 2 + col);
  if (ordinal >= count) return;
  uint8_t item = choices[ordinal];
  bool already = pet.equippedItem(equipmentActionSlot) == item;
  bool ok = already || pet.equipShopProduct(item);
  equipmentActionOpen = false;
  economyNoticeText(already ? "已经装备" : (ok ? "装备完成" : "无法装备"), ok);
}

static void drawEquipmentPetPreview() {
  // 装备页使用约 4 倍于普通详情缩略图的预览，装备槽围绕身体标注。
  if (pmd.loaded) {
    drawPmdActM(pmd, PMD_IDLE, CX, 314, millis(), true, false, 8);
  } else if (mon.loaded) {
    int s = min(8, (int)mon.scale + 2);
    int w = mon.w * s, h = mon.h * s;
    int x = CX - w / 2, y = 314 - h;
...
static void drawEquipmentSlotCard(uint8_t slot) {
  ...
  gfx->fillRoundRect(x, y, EQUIP_BOX_W, EQUIP_BOX_H, 8, UI_WHITE);
  ...
  if (item != EQUIP_EMPTY)
    drawProductIcon(SHOP_CAT_EQUIPMENT, item, x + EQUIP_BOX_W - 22, y + EQUIP_BOX_H / 2, 1);
}

static void renderEquipmentActionDialog() {
  gfx->fillRoundRect(68, 100, 330, 282, 16, UI_WHITE);
  gfx->drawRoundRect(68, 100, 330, 282, 16, UI_INK);
  gfx->setTextColor(UI_INK); gfx->setTextSize(1);
  char title[24]; snprintf(title, sizeof(title), "选择%s", EQUIP_LABELS[equipmentActionSlot]);
  gfx->setCursor(CX - uiTextWidth(title, 6) / 2, 114);
  gfx->print(title);

  uint8_t choices[SHOP_ITEMS_PER_CATEGORY];
  uint8_t count = collectEquipmentChoices(equipmentActionSlot, choices);
  uint8_t pages = (uint8_t)max(1, (int)((count + 5) / 6));
  if (equipmentPickerPage >= pages) equipmentPickerPage = pages - 1;
  if (!count) {
    const char *empty = "仓库暂无可用装备";
    gfx->setTextColor(UI_TRACK);
    gfx->setCursor(CX - uiTextWidth(empty, 6) / 2, 226);
    gfx->print(empty);
  }
  for (uint8_t i = 0; i < 6; ++i) {
    uint8_t ordinal = (uint8_t)(equipmentPickerPage * 6 + i);
    if (ordinal >= count) continue;
    uint8_t item = choices[ordinal];
    int x = 78 + (i % 2) * 160;
    int y = 154 + (i / 2) * 56;
    bool selected = pet.equippedItem(equipmentActionSlot) == item;
    gfx->fillRoundRect(x, y, 150, 46, 8, selected ? C565(0xd2,0xeb,0xdb) : C565(0xee,0xf1,0xf4));
    gfx->drawRoundRect(x, y, 150, 46, 8, selected ? UI_BAR_OK : UI_INK);
    drawProductIcon(SHOP_CAT_EQUIPMENT, item, x + 20, y + 23, 1);
    String name = wifiLabelFit(String(SHOP_PRODUCTS[SHOP_CAT_EQUIPMENT][item].name), 100);
    gfx->setTextColor(UI_INK);
    gfx->setCursor(x + 40, y + 10); gfx->print(name.c_str());
  }
  for (uint8_t i = 0; i < pages; ++i) {
    int dx = CX - ((int)pages - 1) * 9 + i * 18;
    if (i == equipmentPickerPage) gfx->fillCircle(dx, 320, 4, UI_INK);
    else gfx->drawCircle(dx, 320, 3, UI_INK);
  }
  gfx->fillRoundRect(78, 338, 146, 40, 9, C565(0xe0,0xe7,0xee));
  gfx->fillRoundRect(242, 338, 146, 40, 9,
                     pet.equippedItem(equipmentActionSlot) == EQUIP_EMPTY ? UI_TRACK : C565(0xd1,0x75,0x75));
  gfx->drawRoundRect(78, 338, 146, 40, 9, UI_INK);
  gfx->drawRoundRect(242, 338, 146, 40, 9, UI_INK);
  gfx->setTextColor(UI_INK);
  gfx->setCursor(78 + (146 - uiTextWidth("关闭", 6)) / 2, 345); gfx->print("关闭");
  gfx->setTextColor(UI_WHITE);
  gfx->setCursor(242 + (146 - uiTextWidth("卸下", 6)) / 2, 345); gfx->print("卸下");
}
...
void renderCardEquipment() {
  ...
  gfx->setCursor(CX - uiTextWidth(bonus, 6) / 2, 384);
  gfx->print(bonus);
  if (economyMsgUntil && millis() < economyMsgUntil) {
    const char *msg = economyMsgHasText ? economyMsgText : (economyMsgOk ? "操作完成" : "操作失败");
    gfx->fillRoundRect(116, 188, 234, 62, 12, economyMsgOk ? C565(0xd2,0xeb,0xdb) : C565(0xf0,0xd0,0xd0));
    gfx->drawRoundRect(116, 188, 234, 62, 12, UI_INK);
    gfx->setTextColor(UI_INK);
    gfx->setCursor(CX - uiTextWidth(msg, 6) / 2, 205); gfx->print(msg);
  }
...
void renderKeyboard() {
  ...
  for (int i = 0; i < 30; i++) {
    ...
    bool special = (i >= 26 && kbMode) || i >= 28;
    gfx->fillRoundRect(x, y, KB_W - 6, KB_H - 6, 6, special ? UI_BAR_WARN : UI_WHITE);
    gfx->drawRoundRect(x, y, KB_W - 6, KB_H - 6, 6, UI_INK);
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(2);
    if (kbMode && i == 26) {
      gfx->setCursor(x + (KB_W - 6 - uiTextWidth("删除", 6)) / 2, y + 10);
      gfx->print("删除");
    } else if (kbMode && i == 27) {
      gfx->setCursor(x + (KB_W - 6 - uiTextWidth("更多", 6)) / 2, y + 10);
      gfx->print("更多");
    } else if (i < 26 || (!kbMode && i < 28)) {
      const char *keys = wifiKbPage == 0 ? "0123456789ABCDEFGHIJKLMNOP"
                                         : (wifiKbPage == 1 ? "QRSTUVWXYZabcdefghijklmnop"
                                                            : "qrstuvwxyz._-");
      ...
      int cell = kbMode && i == 28 ? 6 : 12;
      gfx->setCursor(x + (KB_W - 6 - uiTextWidth(lab, cell)) / 2, kbMode && i == 28 ? y + 10 : y + KB_H / 2 - 10);
      gfx->print(lab);
    }
  }
  gfx->flush();
...
void battleTap(int16_t x, int16_t y) {
  if (battleResolved) {
    if (battleCaptureUntil && millis() < battleCaptureUntil) return;
    if (battleCatchOffered && !battleCatchDone && y >= 370 && y <= 430) {
      if (x >= 62 && x <= 226) {
        ...
        battleCaptureUntil = battleCatchSuccess ? millis() + 3000 : 0;
        battleDirty = true;
...
void renderBattle() {
  ...
  bool captureFx = battleCaptureUntil && millis() < battleCaptureUntil;
  if (!battleDirty && !captureFx && renderedSession == battleSession &&
  ...
  if (battleResolved) {
    ...
    if (captureFx) {
      // 3 秒捕获庆祝：轻量烟花粒子，完全在圆屏安全区域内。
      uint32_t now = millis();
      for (uint8_t burst = 0; burst < 3; ++burst) {
        int bx = 112 + burst * 120;
        int by = 118 + (burst & 1) * 35;
        for (uint8_t i = 0; i < 12; ++i) {
          float a = (float)i * 0.5235988f;
          int d = 18 + (int)((now / 18 + i * 7 + burst * 13) % 34);
          int px = bx + (int)(cosf(a) * d);
          int py = by + (int)(sinf(a) * d);
          gfx->fillCircle(px, py, 3, (i & 1) ? C565(0xff, 0xd0, 0x43) : C565(0xff, 0x7e, 0x9a));
        }
      }
      gfx->setTextColor(UI_BAR_OK);
      gfx->setTextSize(2);
      const char *celebrate = "捕获成功";
      gfx->setCursor(CX - uiTextWidth(celebrate, 6) / 2, 326);
      gfx->print(celebrate);
    }
  } else {
    gfx->setTextColor(ink);
...
void drawPmdActM(PmdMon &m, uint8_t actId, int cx, int groundY, uint32_t t, bool loop, bool sil, uint8_t maxS) {
  ...
  if (!a.frames) return;
  // 使用小数缩放并在每个源像素边界取整，轮廓会比整倍数大色块更细腻，
  // 同时仍保留原始 PMD 像素画的清晰颜色，不引入模糊滤镜。
  uint16_t targetHeight = maxS >= 8 ? 260 : 204;
  float scale = m.acts[PMD_IDLE].h ? (float)targetHeight / m.acts[PMD_IDLE].h : 5.0f;
  if (scale < 2.0f) scale = 2.0f;
  if (scale > maxS) scale = maxS;
  if (a.h * scale > 250.0f) scale = 250.0f / a.h;
